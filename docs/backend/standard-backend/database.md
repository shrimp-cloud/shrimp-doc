...
- 要点
  ...
  - 布尔类型的含义，均使用正向值值： 正例: `1是0否`，反例: `0是1否`
  - 作为索引的字段，必需 NOT NULL
...
| `xxxx_code_path` | 编码路径                      | 父子关系的的编码路径        | 用于权限控制等逻辑, 如 `aaa/bbb/ccc`          |
| `xxxx_code_show` | 该条数据的唯一标识，不可用于关联，仅用于展示和查询 | 与 deleted 字段组合后唯一 | 用于对外展示的唯一编码。可被修改，但不建议修改。编码生成需要有业务含义 |

注意:
1. 避免使用类似 `code` 无法直接读出字段含义的命名。需要使用可读出含义的字段名
2. 在没有 `xxxx_code_show` 编码展示需求时，尽量不要设计此字段。为了此字段保持修改的能力，不冗余此字段

## 数据状态

| 字段名           | 字段用途        | 内容                                            | 说明                     |
|---------------|-------------|-----------------------------------------------|------------------------|
| `start_time`  | 当前数据的有效开始时间 | 时间                                            | 仅有简单含义的数据有效期描述，使用此字段   |
| `end_time`    | 当前数据的有效结束时间 | 时间                                            | 仅有简单含义的数据有效期描述，使用此字段   |
| `status`      | 当前数据的有效状态   | 1有效,0无效                                       | 仅有简单含义的数据状态，使用此字段      |
| `xxxx_status` | 当前数据的业务状态   | 多状态: 具有业务意义的英文枚举。<br/>布尔状态: 使用`tinyint`, 1是0否 | 具有多业务含义的数据状态，字段使用此方法命名 |

注意:
1. 若仅需要描述数据的有效状态，不使用时间范围控制，使用 `status` 字段描述。
2. 若需要描述数据有效期，使用 `start_time`, `end_time`, `status` ，同时使用定时任务方式驱动 `status` 状态变更
3. 若数据有明确的业务状态含义，使用 `xxxx_status` 描述数据状态，内容为英文枚举，需注明对应系统中的具体枚举类型
