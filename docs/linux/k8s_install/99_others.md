...
> 集群涉及三个相互独立的网段（不能重叠）：
> - **node 网段**：宿主机所在的内网网段
> - **pod 网段**：集群内所有 pod 的 IP 池（对应 kubeadm 配置的 `podSubnet`）
> - **service 网段**：集群内 Service 的虚拟 IP 池（对应 kubeadm 的 `serviceSubnet`，默认 10.96.0.0/12）

- node 网段：192.168.0.0/16
- pod 网段：10.12.0.0/16
- service 网段：10.96.0.0/12（kubeadm 默认值）
