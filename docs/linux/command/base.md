...
| 命令                  | 含义                              |
|---------------------|---------------------------------|
| df -h               | 查看磁盘分区使用情况（-h 人性化显示大小）        |
| du -sh 目录或文件      | 查看指定目录或文件占用空间                  |
| free -h             | 查看内存与交换分区使用情况                  |
| top                 | 实时查看进程与资源占用，按 q 退出             |
| ps -ef \| grep 关键字  | 查看匹配关键字的进程                     |
...
> 注：ntpdate 工具较老，新版系统（如 Rocky 9）推荐使用 chrony：`dnf install -y chrony && systemctl enable --now chronyd`
...
> 更多 vim 详细教程请参考 [菜鸟教程 - Linux vim](https://www.runoob.com/linux/linux-vim.html)
